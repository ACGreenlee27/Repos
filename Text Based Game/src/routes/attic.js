...
import { gameState, typeWriter } from "../../game.js";
...
export async function atticRoute(hasAxe) {
  if (hasAxe === true) {
    ...
    await delay(5000);
    await typeWriter(`The wooden ladder creaks beneath your weight, each step upward swallowed by the thick, suffocating darkness above. 
      Dust drifts down like dead snow, settling in your hair, your lungs, your bones.`);
    await delay(5000);
    await typeWriter(`As your head rises past the threshold, the dim glow of a single, flickering bulb struggles against the shadows. 
    ...
    await delay(5000);
    await typeWriter(`The face is twisted in an expression of terror, mouth slack, eyes wide and unseeing. But it’s not just any face. 
    ...
    await delay(5000);
    await typeWriter(`Scattered across the attic floor, brittle with age, are yellowed news clippings. 
    ...
    await delay(2000);
    await typeWriter("What have you done " + gameState.playerName + "?");
    await delay(5000);
    ...
    if (endingChoice === false) {
      ...
      await delay(2000);
      await typeWriter(`With resolve in your gut, you leave the attic.`);
    }
