...
import { delay, updateKey, typeWriter } from "../../game.js";
...
export async function basementRoute(playerName) {
  ...
  await delay(7000);
  await typeWriter(`You hesitate, but your foot is already on the first step. 
      The wood groans under your weight, the sound stretching unnaturally in the silence.`);
  await delay(5000);
  await typeWriter(`As you descend, the darkness presses closer, swallowing everything beyond the reach of a single, flickering lightbulb hanging from the ceiling. 
  ...
  await delay(5000);
  await typeWriter(
    `Then, at the farthest end of the basement, you see the door.`
  );
  await delay(5000);
  await typeWriter(`It’s an ugly thing—warped, splintered, scarred with deep gouges, as though something inside had spent years trying to claw its way out. 
  ...
  //await typeWriter("Player opens door", playerOpensDoor);
  ...
  if (playerOpensDoor === true) {
    ...
    await delay(5000);
    await typeWriter(`The only light comes from a single, low-burning candle perched on a warped wooden table in the center of the room. 
    ...
    await delay(5000);
    await typeWriter(`Resting in the dust at the center of the table, is a key. 
    ...
    // await typeWriter("Player picks up the key", playerPickedUpKey);
    ...
    if (playerPickedUpKey === true) {
      ...
      await delay(5000);
      await typeWriter(`It looks as if it would fit the closet upstairs... `);
      ...
      await typeWriter(playerName + " returns to the main hall.");
    }
  } else {
    await typeWriter(playerName + " returns to the main hall.");
  }
