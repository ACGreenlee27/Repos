...
export async function shedRoute() {
  ...
  if (gasDecision === true) {
    await typeWriter(`You grab the gas can with both hands, the weight of it grounding you for a moment. 
    ...
    await delay(5000);
    await typeWriter(`The door creaks again as you push it open, the sound like a warning — like something you can’t quite escape. 
  ...
  if (gasDecision === false) {
    ...
    await typeWriter(`You swing it hard against the lock. The sound of metal striking metal rings out in the silence, the shock vibrating through your arm. 
      The lock shatters, its pieces scattering across the floor in a loud, jagged clatter.`);
    await delay(5000);
    await typeWriter(`You kneel down, fingers shaking as you pull away the chain, the lid of the chest groaning open. Inside, you see it — an old revolver, worn but well-maintained, nestled against a faded leather holster. 
  Its cold, dark steel seems to absorb the light around it. You hesitate for only a moment, then grab it, slipping it into your pocket. The weight of it there feels solid, final.`);
    await delay(5000);
    await typeWriter(`You grab the gas can again, the sloshing sound echoing in the stillness. The fire is what you came for, what you need to finish this. With one last glance at the chest — the revolver heavy in your pocket — you turn and leave the shed. 
